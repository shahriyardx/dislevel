...
def get_card(data):
    ...
    background.text((135, 235), data["rep"], font=Font().montserrat(size=24), color="#ffffff", align="center")
    background.text((275, 30), data["name"], font=Font().montserrat(size=50), color="#ffffff")
    background.text((275, 85), f'#{data["descriminator"]}', font=Font().montserrat(size=20), color="#BCBEC0")
    background.text((875, 42), f'#{data["position"]}', font=Font().montserrat(size=45), color="#ffffff", align="right")

    background.paste(prog_back, (275, 220))
    prog_bar.bar((0, 0), 580, 30, 40, "#A7A9AC91", radius=8)
    background.paste(prog_bar, (277, 222))
    background.text((570, 226), f"{data['current_user_exp']} / {data['next_level_exp']}", font=Font().montserrat(size=20), color="white", align="center")

    background.text((275, 265), f"Level : {data['level']}", font=Font().montserrat(size=18), color="white", align="left")
    if data['next_role']:
        background.text((863, 265), f"Next Role : {data['next_role']}", font=Font().montserrat(size=18), color="white", align="right")
